...
import { Language, RecentSyncDetails, Word } from "@/types";
...
export async function storeWords(language: Language, words: string[]) {
  const db = await initDB();
  const tx = db.transaction(WORDS_STORE_NAME, "readwrite");
  const store = tx.objectStore(WORDS_STORE_NAME);

  await Promise.all(words.map((word) => store.add({ word, lang: language })));
